...
/* $Id: cDeep_Compiler.class.php,v 1.386 2006/11/30 17:01:28 mohrt Exp $ */

/**
 * Template compiling class
 * @package cDeep
 */
class cDeep_Compiler extends cDeep {
	...
	*/
	...
	function _compile_file($resource_name, $source_content, &$compiled_content)
	{
		...
		if ($this->security) {
			...
			if ($this->php_handling == cDeep_PHP_ALLOW &&
		...
		for ($curr_tb = 0, $for_max = count($text_blocks); $curr_tb < $for_max; $curr_tb++) {
			...
			if (preg_match_all('~(<\?(?:\w+|=)?|\?>|language\s*=\s*[\"\']?php[\"\']?)~is', $text_blocks[$curr_tb], $sp_match)) {
				...
				usort($sp_match[1], '_cDeep_sort_length');
				for ($curr_sp = 0, $for_max2 = count($sp_match[1]); $curr_sp < $for_max2; $curr_sp++) {
					$text_blocks[$curr_tb] = str_replace($sp_match[1][$curr_sp],'%%%cDeepSP'.$curr_sp.'%%%',$text_blocks[$curr_tb]);
				}
				/* process each one */
				for ($curr_sp = 0, $for_max2 = count($sp_match[1]); $curr_sp < $for_max2; $curr_sp++) {
					if ($this->php_handling == cDeep_PHP_PASSTHRU) {
						/* echo php contents */
						$text_blocks[$curr_tb] = str_replace('%%%cDeepSP'.$curr_sp.'%%%', '<?php echo \''.str_replace("'", "\'", $sp_match[1][$curr_sp]).'\'; ?>'."", $text_blocks[$curr_tb]);
					} else if ($this->php_handling == cDeep_PHP_QUOTE) {
						/* quote php tags */
						$text_blocks[$curr_tb] = str_replace('%%%cDeepSP'.$curr_sp.'%%%', htmlspecialchars($sp_match[1][$curr_sp]), $text_blocks[$curr_tb]);
					} else if ($this->php_handling == cDeep_PHP_REMOVE) {
						/* remove php tags */
						$text_blocks[$curr_tb] = str_replace('%%%cDeepSP'.$curr_sp.'%%%', '', $text_blocks[$curr_tb]);
					} else {
						/* cDeep_PHP_ALLOW, but echo non php starting tags */
						$sp_match[1][$curr_sp] = preg_replace('~(<\?(?!php|=|$))~i', '<?php echo \'\\1\'?>'."", $sp_match[1][$curr_sp]);
						$text_blocks[$curr_tb] = str_replace('%%%cDeepSP'.$curr_sp.'%%%', $sp_match[1][$curr_sp], $text_blocks[$curr_tb]);
					}
				}
			}
		}
		...
		if (count($this->_plugins['postfilter']) > 0) {
			foreach ($this->_plugins['postfilter'] as $filter_name => $postfilter) {
				...
				if ($postfilter[3] || is_callable($postfilter[0])) {
					...
					$this->_trigger_fatal_error("cDeep plugin error: postfilter '$filter_name' is not implemented");
				}
			}
		}

		// put header at the top of the compiled template
		$template_header = "<?php /* cDeep-".$this->_version.", generated on ".strftime("%Y-%m-%d %H:%M:%S")."\n";
		$template_header .= "         template ".strtr(urlencode($resource_name), array('%2F'=>'/', '%3A'=>':'))." */ ?>\n";
		...
		if (count($this->_plugin_info)) {
			...
			$_plugins_params .= '))';
			$plugins_code = "<?php require_once(cDeep_CORE_DIR . 'core.load_plugins.php');\ncDeep_core_load_plugins($_plugins_params, \$this); ?>";
			$template_header .= $plugins_code;
			$this->_plugin_info = array();
			$this->_plugins_code = $plugins_code;
		}

		if ($this->_init_cDeep_vars) {
			$template_header .= "<?php require_once(cDeep_CORE_DIR . 'core.assign_cDeep_interface.php');\ncDeep_core_assign_cDeep_interface(null, \$this); ?>";
			$this->_init_cDeep_vars = false;
		}
	...
	function _compile_compiler_tag($tag_command, $tag_args, &$output)
	{
		...
		*/
			...
			$plugin_func = 'cDeep_compiler_' . $tag_command;
			if (!is_callable($plugin_func)) {
	...
	function _compile_block_tag($tag_command, $tag_args, $tag_modifier, &$output)
	{
		...
		*/
			...
			$plugin_func = 'cDeep_block_' . $tag_command;
	...
	function _compile_custom_tag($tag_command, $tag_args, $tag_modifier, &$output)
	{
		...
		*/
			...
			$plugin_func = 'cDeep_function_' . $tag_command;
			if (!function_exists($plugin_func)) {
				$message = "plugin function $plugin_func() not found in $plugin_file\n";
				$have_function = false;
			} else {
	...
	function _compile_insert_tag($tag_args)
	{
		...
		return "<?php require_once(cDeep_CORE_DIR . 'core.run_insert_handler.php');\necho cDeep_core_run_insert_handler($_params, \$this); ?>" . $this->_additional_newline;
	}
	...
	function _compile_include_tag($tag_args)
	{
		...
		$_params = "array('cDeep_include_tpl_file' => " . $include_file . ", 'cDeep_include_vars' => array(".implode(',', (array)$arg_list)."))";
		$output .= "\$this->_cDeep_include($_params);\n" .
	...
	function _compile_include_php_tag($tag_args)
	{
		...
		$_params = "array('cDeep_file' => " . $attrs['file'] . ", 'cDeep_assign' => '$assign_var', 'cDeep_once' => $once_var, 'cDeep_include_vars' => array(".implode(',', $arg_list)."))";

		return "<?php require_once(cDeep_CORE_DIR . 'core.cDeep_include_php.php');\ncDeep_core_cDeep_include_php($_params, \$this); ?>" . $this->_additional_newline;
	}
	...
	function _compile_capture_tag($start, $tag_args = '')
	{
		...
		if ($start) {
			...
			$output = "<?php \$this->_cDeep_vars['capture'][$buffer] = ob_get_contents(); ";
			if (isset($assign)) {
	...
	function _compile_arg_list($type, $name, $attrs, &$cache_code) {
		...
		&& is_array($this->_plugins[$type][$name][5])
			...
			$cache_code = "\$_cache_attrs =& \$this->_cDeep_cache_attrs('$this->_cache_serial','$_count');";
	...
	function _parse_var_props($val)
	{
		...
		if(preg_match('~^(' . $this->_obj_call_regexp . '|' . $this->_dvar_regexp . ')(' . $this->_mod_regexp . '*)$~', $val, $match)) {
			...
			$modifiers = $match[2];
			if (!empty($this->default_modifiers) && !preg_match('~(^|\|)cDeep:nodefaults($|\|)~',$modifiers)) {
	...
	function _parse_var($var_expr)
	{
		...
		if(!$_has_math) {
			...
			/* Handle $cDeep.* variable references as a special case. */
			if ($_var_name == 'cDeep') {
				/*
				* If the reference could be compiled, use the compiled output;
				* otherwise, fall back on the $cDeep variable generated at
				* run-time.
				*/
				if (($cDeep_ref = $this->_compile_cDeep_ref($_indexes)) !== null) {
					$_output = $cDeep_ref;
				} else {
					$_var_name = substr(array_shift($_indexes), 1);
					$_output = "\$this->_cDeep_vars['$_var_name']";
				}
	...
	/**
     * Compiles references of type $cDeep.foo
     *
     * @param string $indexes
     * @return string
     */
	function _compile_cDeep_ref(&$indexes)
	{
		/* Extract the reference name. */
		$_ref = substr($indexes[0], 1);
		foreach($indexes as $_index_no=>$_index) {
			if (substr($_index, 0, 1) != '.' && $_index_no<2 || !preg_match('~^(\.|\[|->)~', $_index)) {
				$this->_syntax_error('$cDeep' . implode('', array_slice($indexes, 0, 2)) . ' is an invalid reference', E_USER_ERROR, __FILE__, __LINE__);
			}
		}
		...
		switch ($_ref) {
			...
			default:
				$this->_syntax_error('$cDeep.' . $_ref . ' is an unknown reference', E_USER_ERROR, __FILE__, __LINE__);
				break;
		}

		if (isset($_max_index) && count($indexes) > $_max_index) {
			$this->_syntax_error('$cDeep' . implode('', $indexes) .' is an invalid reference', E_USER_ERROR, __FILE__, __LINE__);
		}
	...
	function _push_cacheable_state($type, $name) {
		...
		if (!isset($this->_cache_serial)) $this->_cache_serial = md5(uniqid('cDeep'));
 ...
 * @return 0|-1|1
 */
function _cDeep_sort_length($a, $b)
{
	if($a == $b)
	return 0;
